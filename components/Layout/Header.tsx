...
import { Search, Mic, User, Phone, ChevronDown, Menu, X, Star, Sparkles, ChevronRight, MapPin, ArrowRight, Flame, Compass } from 'lucide-react';
...
interface HeaderProps {
  onNavigate?: (page: 'home' | 'booking' | 'india') => void;
}
      ...
      <header className="fixed top-0 left-0 right-0 z-50 w-full text-white font-medium bg-[#0E1E2C] shadow-xl transition-all duration-300">
        ...
        <div className="max-w-[90rem] mx-auto px-4 py-1 md:py-1 flex flex-col md:flex-row md:items-center justify-between gap-1.5 md:gap-3 relative z-50">
          ...
          <div className="hidden md:flex items-center gap-2 md:gap-5 ml-auto md:ml-0 min-w-max">
            ...
            {/* Phone Number */}
            <div className="hidden lg:flex items-center gap-3 border border-white/20 rounded-full p-1 pr-4 hover:border-[#FFD801] transition-all cursor-pointer group bg-transparent">
               ...
               <span className="text-base font-bold text-white tracking-wide">95111 71797</span>
            </div>
            
            <div className="flex items-center gap-2 cursor-pointer group">
              <div className="w-11 h-11 rounded-full bg-white/10 flex items-center justify-center group-hover:bg-white/20 transition-all border border-white/10">
              ...
              {menuItems.map((item) => (
                <li 
                    ...
                    className={`group static flex items-center gap-1.5 cursor-pointer transition-colors h-full 
                        ${(item === 'India' && isIndiaHovered) || (item === 'World' && isWorldHovered) ? 'text-[#FFD801]' : 'hover:text-[#FFD801]'}`}
                    onMouseEnter={() => {
                ...
                >
                  ...
                  {(item === 'India' || item === 'World' || item === 'Group Tours' || item === 'Specialty Tours' || item === 'More') && 
                    <ChevronDown className={`w-3.5 h-3.5 opacity-70 transition-transform 
                        ${(item === 'India' && isIndiaHovered) || (item === 'World' && isWorldHovered) ? 'rotate-180' : 'group-hover:rotate-180'}`} />
                  }
            ...
            {menuItems.map((item, index) => {
                ...
                return (
                    <li key={index} className="border-b border-slate-100">
                        <div
                            className="flex justify-between items-center px-5 py-4 cursor-pointer hover:bg-slate-50 transition-colors group"
                            onClick={() => {
                                if (isMegaMenu) {
                        ...
                        >
                            <span className={`text-[15px] font-bold ${isExpanded ? 'text-[#FFD801]' : 'text-slate-700'} group-hover:text-[#0E1E2C]`}>{item}</span>
                            {isMegaMenu ? (
